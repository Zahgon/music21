...
class _EnvironmentCore:
    ...
    def __setitem__(self, key, value):
        ...
        if not valid:
            raise EnvironmentException(f"{value} is not an acceptable value for preference: {key}")
    ...
    def read(self, filePath=None):
        ...
        try:
            ...
            raise EnvironmentException(f"Cannot parse file {filePath}: {str(pe)}") from pe
...
class Environment:
    ...
    @overload
    def getTempFile(
        self, suffix: str = "", returnPathlib: t.Literal[True] = True
    ) -> pathlib.Path: ...
    ...
    def __setitem__(self, key, value):
        ...
        # when setting a local corpus setting, if not a list, append
            ...
            if not isinstance(value, dict):
                raise UserSettingsException("localCorporaSettings must be provided as a dict.")
            if value:
...
class Test(unittest.TestCase):
    ...
    @unittest.skipUnless(
        common.getPlatform() in ["nix", "darwin"], "os.getuid can be called only on Unix platforms"
    )
